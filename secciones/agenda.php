...
<div class="agenda-section">
    <div class="agenda-toolbar">
        <div class="agenda-fecha-nav">
            <button class="btn-nav" onclick="cambiarFecha(-1)"><i class="fa-solid fa-chevron-left"></i></button>
            <span class="fecha-actual" id="fechaDisplay"><?php echo date('d \d\e F \d\e Y', strtotime($fecha)); ?></span>
            <button class="btn-nav" onclick="cambiarFecha(1)"><i class="fa-solid fa-chevron-right"></i></button>
        </div>
        <button class="btn-nueva-cita" onclick="abrirModal()"><i class="fa-solid fa-plus"></i> Nueva Cita</button>
    </div>
            ...
            <?php for ($h = 6; $h <= 22; $h++): ?>
                ...
                <div class="hora-fila <?php echo $tieneCitas ? 'tiene-citas' : ''; ?>">
                    ...
                    <div class="hora-contenido">
                        <?php if ($tieneCitas): foreach ($citasPorHora[$h] as $cita): ?>
                            <div class="cita-bloque" style="border-left:4px solid <?php echo htmlspecialchars($cita['color']); ?>">
                                ...
                                <div class="cita-horario"><?php echo substr($cita['hora_inicio'],0,5); ?> - <?php echo substr($cita['hora_fin'],0,5); ?></div>
                                <?php if ($cita['descripcion']): ?><div class="cita-desc"><?php echo htmlspecialchars($cita['descripcion']); ?></div><?php endif; ?>
                                <button class="btn-eliminar-cita" onclick="eliminarCita(<?php echo $cita['id']; ?>)"><i class="fa-solid fa-xmark"></i></button>
                            </div>
                        <?php endforeach; endif; ?>
                        <button class="btn-add-hora" onclick="abrirModal('<?php echo $horaStr; ?>')"><i class="fa-solid fa-plus"></i></button>
                    </div>
                </div>
            <?php endfor; ?>
        </div>
...
<div class="modal-overlay" id="modalCita" style="display:none;">
    <div class="modal-box">
        ...
        <div class="modal-body">
            ...
            <div class="campo"><label>Descripción</label><textarea id="input-desc" placeholder="Detalles opcionales..."></textarea></div>
            <div class="campo-row">
...
<style>
.agenda-section{display:flex;flex-direction:column;gap:20px}
.agenda-toolbar{background:var(--card);border:2px solid var(--black);border-radius:18px;box-shadow:5px 5px 0 var(--black);padding:16px 24px;display:flex;justify-content:space-between;align-items:center;gap:16px}
.agenda-fecha-nav{display:flex;align-items:center;gap:16px}
.btn-nav{width:36px;height:36px;border-radius:10px;border:2px solid var(--black);background:var(--bg);cursor:pointer;font-size:14px;display:flex;align-items:center;justify-content:center;transition:all .15s;box-shadow:2px 2px 0 var(--black)}
.btn-nav:hover{background:var(--black);color:var(--card);transform:translateY(-1px)}
.fecha-actual{font-weight:700;font-size:15px;color:var(--black);font-family:var(--font);min-width:200px;text-align:center}
.btn-nueva-cita{background:var(--black);color:var(--card);border:2px solid var(--black);border-radius:12px;padding:10px 20px;font-family:var(--font);font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:8px;box-shadow:3px 3px 0 #7b2cbf;transition:all .15s}
.btn-nueva-cita:hover{transform:translateY(-2px);box-shadow:5px 5px 0 #7b2cbf}
.agenda-grid-wrapper{background:var(--card);border:2px solid var(--black);border-radius:18px;box-shadow:5px 5px 0 var(--black);overflow:hidden}
.agenda-grid{display:flex;flex-direction:column}
.hora-fila{display:flex;align-items:stretch;border-bottom:1.5px solid var(--line);min-height:64px;transition:background .15s}
.hora-fila:last-child{border-bottom:none}
.hora-fila:hover{background:var(--bg)}
.hora-label{width:100px;flex-shrink:0;padding:12px 16px;font-size:12px;font-weight:700;color:var(--muted);font-family:var(--font);border-right:2px solid var(--line);display:flex;align-items:center}
.hora-contenido{flex:1;padding:8px 12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;position:relative}
.cita-bloque{background:var(--bg);border:1.5px solid var(--black);border-radius:10px;padding:8px 12px;flex:1;min-width:180px;position:relative;box-shadow:2px 2px 0 var(--black)}
.cita-titulo{font-weight:700;font-size:13px;color:var(--black);font-family:var(--font)}
.cita-horario{font-size:11px;color:var(--muted);margin-top:2px;font-family:var(--font)}
.cita-desc{font-size:12px;color:var(--muted);margin-top:4px;font-family:var(--font)}
.btn-eliminar-cita{position:absolute;top:6px;right:6px;background:none;border:none;cursor:pointer;color:var(--muted);font-size:12px;padding:2px 4px;border-radius:4px;transition:all .15s}
.btn-eliminar-cita:hover{background:#fecaca;color:#dc2626}
.btn-add-hora{width:28px;height:28px;border-radius:8px;border:1.5px dashed var(--muted);background:transparent;cursor:pointer;font-size:12px;color:var(--muted);display:flex;align-items:center;justify-content:center;opacity:0;transition:all .15s;flex-shrink:0}
.hora-fila:hover .btn-add-hora{opacity:1}
.btn-add-hora:hover{border-color:var(--black);color:var(--black);background:var(--card)}
...
.campo input:focus,.campo textarea:focus{box-shadow:3px 3px 0 var(--black)}
